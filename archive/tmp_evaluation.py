...
def plot_ppv():
	...
	def get_all_predictions():
		...
		for sequence_name in sequence_names:
			...
			all_predictions['pconsc'].extend((100 * float(k) / L, 0 < contact_matrix[i-1,j-1] <= 8) for (k, (i, j, s)) in enumerate(itertools.islice(data_io.read_residue_pairs('pconsc_predictions/' + sequence_name, ' '), int(max_predictions_fraction * L))))
	...
	def accumulate_predictions(predictions):
		...
		for method in itertools.chain(methods, ['pconsc']):
	...
	def plot_ppv_curves(ppv):
		for (method, name, style) in itertools.chain(zip(methods, method_names, method_styles), [('pconsc', 'PconsC', 'r-')]):
			X, Y = ppv[method]
	...
	try:
		predictions = pickle.load(open('evaluation_predictions_tmp.pickled', 'rb'))
	except:
		predictions = get_all_predictions()
		pickle.dump(predictions, open('evaluation_predictions_tmp.pickled', 'wb'))

	try:
		ppv = pickle.load(open('evaluation_ppv_tmp.pickled', 'rb'))
	except:
		ppv = accumulate_predictions(predictions)
		pickle.dump(ppv, open('evaluation_ppv_tmp.pickled', 'wb'))
